"""
Ground-truth reward for Gymnasium-Robotics Point Maze Large Diverse_G (sparse only).
...
class PointMazeLargeDiverseGV3RewardFunction(RewardFunction):
    """
    Sparse reward for achieved vs desired goal.

    Assumptions:
    - prev_obs and next_obs are PointMazeLargeDiverseGV3Observation instances.
    ...
    def calculate_reward(self, prev_obs, action, next_obs):
        assert isinstance(next_obs, PointMazeLargeDiverseGV3Observation), (
            "next_obs must be PointMazeLargeDiverseGV3Observation."
        )
        assert isinstance(prev_obs, PointMazeLargeDiverseGV3Observation), (
            "prev_obs must be PointMazeLargeDiverseGV3Observation."
        )
