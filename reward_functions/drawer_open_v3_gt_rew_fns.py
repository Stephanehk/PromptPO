"""
Ground-truth reward for Meta-World drawer-open-v3 (MT1).

Delegates to SawyerDrawerEnvV3.evaluate_state so the scalar matches the env step reward
...
class DrawerOpenV3RewardFunction(RewardFunction):
    def set_specific_reward(self, _reward_id):
        pass

    def calculate_reward(self, prev_obs, action, next_obs):
        assert isinstance(next_obs, MetaWorldDrawerOpenV3Observation), (
            "next_obs must be MetaWorldDrawerOpenV3Observation."
        )
